"use client";
import React, { useState, useEffect } from "react";
import { Link } from "@/i18n/navigation";
import { useLocale } from "next-intl";
import { getPosts } from "@/libs/api"; // Import fungsi getPosts

const FloatButton = () => {
  const locale = useLocale();
  const [whatsappNumber, setWhatsappNumber] = useState("");
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadData = async () => {
      try {
        // Menggunakan getPosts untuk mengambil data settings
        const settingsData = await getPosts("settings");

        // Ambil nomor WhatsApp dari data social
        const whatsapp = settingsData?.data?.social?.whatsapp;
        if (whatsapp) {
          setWhatsappNumber(whatsapp);
        }
      } catch (error) {
        console.error("Error loading data:", error);
      } finally {
        setIsLoading(false);
      }
    };

    loadData();
  }, []);

  // Fungsi untuk membersihkan dan memformat nomor WhatsApp
  const cleanWhatsAppNumber = (number) => {
    if (!number) return "";

    // Hapus semua karakter non-digit
    let cleaned = number.replace(/\D/g, "");

    // Jika diawali dengan 0, ganti dengan 62
    if (cleaned.startsWith("0")) {
      cleaned = "62" + cleaned.substring(1);
    }
    // Jika diawali dengan +62, hapus tanda +
    else if (cleaned.startsWith("62")) {
      // Sudah dalam format 62, tidak perlu perubahan
      cleaned = cleaned;
    }
    // Jika diawali dengan +, hapus tanda +
    else if (cleaned.startsWith("+")) {
      cleaned = cleaned.substring(1);
    }

    return cleaned;
  };

  // Format URL WhatsApp
  const whatsappUrl = whatsappNumber
    ? `https://wa.me/${cleanWhatsAppNumber(whatsappNumber)}`
    : "#";

  // Jika masih loading, jangan render apa-apa atau render loading state
  if (isLoading) {
    return (
      <div className="flex flex-col fixed z-[100] bottom-[30px] right-[30px] float-button">
        <div className="bg-gray-400 w-[42px] h-[42px] lg:w-[52px] lg:h-[52px] flex flex-col items-center justify-center rounded-full cursor-not-allowed relative bottom-0 right-0 shadow-[0px_4px_10px_0px_#0000001A] animate-pulse">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="scale-[135%] opacity-50"
          >
            <path
              d="M17 2.91005C16.0831 1.98416 14.991 1.25002 13.7875 0.750416C12.584 0.250812 11.2931 -0.00426317 9.99 5.38951e-05C4.53 5.38951e-05 0.0800002 4.45005 0.0800002 9.91005C0.0800002 11.6601 0.54 13.3601 1.4 14.8601L0 20.0001L5.25 18.6201C6.7 19.4101 8.33 19.8301 9.99 19.8301C15.45 19.8301 19.9 15.3801 19.9 9.92005C19.9 7.27005 18.87 4.78005 17 2.91005ZM9.99 18.1501C8.51 18.1501 7.06 17.7501 5.79 17.0001L5.49 16.8201L2.37 17.6401L3.2 14.6001L3 14.2901C2.17755 12.9771 1.74092 11.4593 1.74 9.91005C1.74 5.37005 5.44 1.67005 9.98 1.67005C12.18 1.67005 14.25 2.53005 15.8 4.09005C16.5676 4.85392 17.1759 5.7626 17.5896 6.76338C18.0033 7.76417 18.2142 8.83714 18.21 9.92005C18.23 14.4601 14.53 18.1501 9.99 18.1501ZM14.51 11.9901C14.26 11.8701 13.04 11.2701 12.82 11.1801C12.59 11.1001 12.43 11.0601 12.26 11.3001C12.09 11.5501 11.62 12.1101 11.48 12.2701C11.34 12.4401 11.19 12.4601 10.94 12.3301C10.69 12.2101 9.89 11.9401 8.95 11.1001C8.21 10.4401 7.72 9.63005 7.57 9.38005C7.43 9.13005 7.55 9.00005 7.68 8.87005C7.79 8.76005 7.93 8.58005 8.05 8.44005C8.17 8.30005 8.22 8.19005 8.3 8.03005C8.38 7.86005 8.34 7.72005 8.28 7.60005C8.22 7.48005 7.72 6.26005 7.52 5.76005C7.32 5.28005 7.11 5.34005 6.96 5.33005H6.48C6.31 5.33005 6.05 5.39005 5.82 5.64005C5.6 5.89005 4.96 6.49005 4.96 7.71005C4.96 8.93005 5.85 10.1101 5.97 10.2701C6.09 10.4401 7.72 12.9401 10.2 14.0101C10.79 14.2701 11.25 14.4201 11.61 14.5301C12.2 14.7201 12.74 14.6901 13.17 14.6301C13.65 14.5601 14.64 14.0301 14.84 13.4501C15.05 12.8701 15.05 12.3801 14.98 12.2701C14.91 12.1601 14.76 12.1101 14.51 11.9901Z"
              fill="white"
            />
          </svg>
          <div className="shadow-[0px_4px_10px_0px_#0000001A] w-[120px] bg-white flex flex-col items-center justify-center float-text rounded-[5px] absolute left-[-90%] opacity-0 invisible transition-all ease duration-300 translate-x-[-50%]">
            <p className="uppercase font-raleway font-normal text-navyblue tracking-[1px] leading-[1] px-2 py-2 text-xs">
              {locale === "en" ? "Loading..." : "Memuat..."}
            </p>
          </div>
        </div>
      </div>
    );
  }

  return (
    <>
      <div className="flex flex-col fixed z-[100] bottom-[30px] right-[30px] float-button">
        {/* Float Default Button */}
        <Link
          href={whatsappUrl}
          target="_blank"
          className={`bg-tosca w-[42px] h-[42px] lg:w-[52px] lg:h-[52px] flex flex-col items-center justify-center rounded-full cursor-pointer relative bottom-0 right-0 shadow-[0px_4px_10px_0px_#0000001A] float-default transition-all duration-300 group ${
            !whatsappNumber ? "pointer-events-none opacity-50" : ""
          }`}
        >
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="scale-[135%]"
          >
            <path
              d="M17 2.91005C16.0831 1.98416 14.991 1.25002 13.7875 0.750416C12.584 0.250812 11.2931 -0.00426317 9.99 5.38951e-05C4.53 5.38951e-05 0.0800002 4.45005 0.0800002 9.91005C0.0800002 11.6601 0.54 13.3601 1.4 14.8601L0 20.0001L5.25 18.6201C6.7 19.4101 8.33 19.8301 9.99 19.8301C15.45 19.8301 19.9 15.3801 19.9 9.92005C19.9 7.27005 18.87 4.78005 17 2.91005ZM9.99 18.1501C8.51 18.1501 7.06 17.7501 5.79 17.0001L5.49 16.8201L2.37 17.6401L3.2 14.6001L3 14.2901C2.17755 12.9771 1.74092 11.4593 1.74 9.91005C1.74 5.37005 5.44 1.67005 9.98 1.67005C12.18 1.67005 14.25 2.53005 15.8 4.09005C16.5676 4.85392 17.1759 5.7626 17.5896 6.76338C18.0033 7.76417 18.2142 8.83714 18.21 9.92005C18.23 14.4601 14.53 18.1501 9.99 18.1501ZM14.51 11.9901C14.26 11.8701 13.04 11.2701 12.82 11.1801C12.59 11.1001 12.43 11.0601 12.26 11.3001C12.09 11.5501 11.62 12.1101 11.48 12.2701C11.34 12.4401 11.19 12.4601 10.94 12.3301C10.69 12.2101 9.89 11.9401 8.95 11.1001C8.21 10.4401 7.72 9.63005 7.57 9.38005C7.43 9.13005 7.55 9.00005 7.68 8.87005C7.79 8.76005 7.93 8.58005 8.05 8.44005C8.17 8.30005 8.22 8.19005 8.3 8.03005C8.38 7.86005 8.34 7.72005 8.28 7.60005C8.22 7.48005 7.72 6.26005 7.52 5.76005C7.32 5.28005 7.11 5.34005 6.96 5.33005H6.48C6.31 5.33005 6.05 5.39005 5.82 5.64005C5.6 5.89005 4.96 6.49005 4.96 7.71005C4.96 8.93005 5.85 10.1101 5.97 10.2701C6.09 10.4401 7.72 12.9401 10.2 14.0101C10.79 14.2701 11.25 14.4201 11.61 14.5301C12.2 14.7201 12.74 14.6901 13.17 14.6301C13.65 14.5601 14.64 14.0301 14.84 13.4501C15.05 12.8701 15.05 12.3801 14.98 12.2701C14.91 12.1601 14.76 12.1101 14.51 11.9901Z"
              fill="white"
            />
          </svg>
          <div className="shadow-[0px_4px_10px_0px_#0000001A] w-[120px] bg-white flex flex-col items-center justify-center float-text rounded-[5px] absolute left-[-90%] opacity-0 invisible transition-all ease duration-300 translate-x-[-50%] group-hover:opacity-100 group-hover:visible group-hover:left-[-135%]">
            <p className="uppercase font-raleway font-normal text-navyblue tracking-[1px] leading-[1] px-2 py-2 text-xs">
              {locale === "en" ? "Contact Us" : "Hubungi Kami"}
            </p>
            <div
              style={{ clipPath: "polygon(0 0, 0% 100%, 100% 50%)" }}
              className="bg-white w-[17px] h-[17px] absolute top-[-2px] right-[-4px] transform translate-1/2"
            ></div>
          </div>
        </Link>
      </div>
    </>
  );
};

export default FloatButton;
